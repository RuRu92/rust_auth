...
use authenticator::resource::customer;
...
async fn test_signup_and_login() {
    let db = Arc::new(db::DB::init(
        "mysql://root:password@localhost:3306/auth",
    )); // Or use a test DB

    let realm_settings_provider = Arc::new(RealmSettingProvider::init(db.clone()));
    let app_sate = AppState {
        realm_settings_provider,
        execution_context: db::ExecutionContext { db },
    };

    let app = test::init_service(
        App::new()
            .app_data(web::Data::new(app_sate))
            .route("/signup", web::post().to(customer::create)),
    )
    .await;
