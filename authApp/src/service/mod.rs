...
pub mod customer_service {
    ...
    use crate::domain::realm::{
        InternalRealmSettings, RealmName, RealmSettings, UserRealmSettings,
    };
    ...
    impl AuthenticatorService {
        pub fn initialise_token(
            realm: &str,
            data: &User,
            realm_settings_provider: &RealmSettingProvider,
        ) -> String {
    ...
    impl CustomerService {
        pub fn fetch_users(
            realm: &RealmName,
            db_context: &DB,
        ) -> APIResult<Vec<UserWithAddress>, APIError> {
            db_context.in_transaction(AccessMode::ReadWrite, |tx| {
                UserStorage::get_users(realm, tx)
            })
        }

        pub fn fetch_user(user_id: &String, db_context: &DB) -> APIResult<Option<User>> {
        ...
        fn handle_create_user(
        ...
        ) -> impl FnMut(&mut Transaction) -> APIResult<(String, String), mysql::Error> {
            move |tx: &mut Transaction| {
                ...
                let address_id = AddressStorage::create_from(
                    (user_data.realm.clone(), user_id.to_owned(), address),
                    tx,
                )
